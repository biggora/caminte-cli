...
module.exports = {
        ...
        while ((result = regex.exec(sql)) !== null) {
            ...
            if (tname) {
                ...
                if (type === 3 || type === 2 || type === 1) {
                    cindexes = self.findIndexes(table.tableName, sql, type);
                    uindexes = self.findUniqueIndexes(table.tableName, sql);
                }
    ...
    renderFields : function (fields, cb) {
        ...
        fields.forEach(function (field) {
            ...
            if (field.null === false) {
                row += ', "null": false'; // http://www.camintejs.com/en/guide#define-indices
                row += ', "allowNull": false'; // https://github.com/biggora/caminte/blob/master/lib/adapters/mysql.js#L774
            } else {
                row += ', "null": true';
                row += ', "allowNull": true';
            }
        ...
        if (ssql && ssql.length) {
            ...
            for (var si = 0; si < ssql.length; si++) {
                ...
                if (/^`([a-zA-Z0-9_]+)`\s+([a-zA-Z0-9\(\)]+)/gi.test(tsql) ||
                    ...
                    if (/AUTO_INCREMENT/gi.test(tsql)) {
